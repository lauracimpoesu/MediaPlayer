...
function forward() {
  document.querySelector("#controllo").currentTime += 1
}
function back() {
  document.querySelector("#controllo").currentTime -= 3
}
function speedItUp() {
  document.querySelector("#controllo").playbackRate = 2
}
...
var displayMyMedia = {
  ...
  displayMagic() {
    canciones = ""
    misvideos = ""
    for (i = 0; i < displayMyMedia.swissMedia.theAudio.length; i++) {
      canciones +=
        "<li id='firstList' class='list-group-item' onclick=\"displayMyMedia.swissMedia.startFunctions('" +
        displayMyMedia.swissMedia.theAudio[i].file +
        "', '" +
        displayMyMedia.swissMedia.types.AUDIO +
        "')\">" +
        displayMyMedia.swissMedia.theAudio[i].title +
        "</li>"
    }
    for (i = 0; i < displayMyMedia.swissMedia.theVideo.length; i++) {
      misvideos +=
        "<li id='secondList' class='list-group-item' onclick=\"displayMyMedia.swissMedia.startFunctions('" +
        displayMyMedia.swissMedia.theVideo[i].file +
        "', '" +
        displayMyMedia.swissMedia.types.VIDEO +
        "')\">" +
        displayMyMedia.swissMedia.theVideo[i].title +
        "</li>"
    }
